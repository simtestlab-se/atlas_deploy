...
//    SPI_MASTER_Transmit(&SPI_MASTER_0, txBuff_cmd, sizeof(CMDPKT_LEN));
    spi_write(txBuff_cmd,CMDPKT_LEN);
...
{
    uint8_t txBuff_data[TOTAL_AD_IC][DATAPKT_LEN];    // 6 Data + 2 DPEC per IC

    for (int ic = 0; ic < TOTAL_AD_IC; ic++)   /* The first configuration written is received by the last IC in the daisy chain */
    {
        int iv = (TOTAL_AD_IC - 1) - ic;    // Inverted index to get data from the back
...
//    SPI_MASTER_Transmit(&SPI_MASTER_0, (uint8_t *)txBuff_data, DATAPKT_LEN * TOTAL_AD_IC);
    spi_write((uint8_t *)txBuff_data,DATAPKT_LEN * TOTAL_AD_IC);
}


void bms_spiReceiveData(uint8_t rxData[TOTAL_AD_IC][DATA_LEN], uint16_t rxPec[TOTAL_AD_IC], uint8_t rxCc[TOTAL_AD_IC])
{
    uint8_t rawRxData[TOTAL_AD_IC][DATAPKT_LEN];

//    uint8_t dummy = 0x00;
          // Full-duplex transfer: send dummy, receive real
//    SPI_MASTER_Transfer(&SPI_MASTER_0, &dummy, (uint8_t *)rawRxData, DATAPKT_LEN * TOTAL_AD_IC);

    spi_read((uint8_t *)rawRxData,DATAPKT_LEN * TOTAL_AD_IC);

    for (int ic = 0; ic < TOTAL_AD_IC; ic++)     /* executes for each ic in the daisy chain and packs the data */
    {
...
//        uint8_t dummy = 0x00;
              // Full-duplex transfer: send dummy, receive real
//        SPI_MASTER_Transfer(&SPI_MASTER_0, &dummy, &buff, 1);
    	spi_read(&buff,1);
    }
